...
- [[#sly][SLY]]
- [[#programming-things][Programming things]]
...
Packages and configurations are not in alphabetical order! If they
where things wouldn't work properly.
...
#+end_src
Mu4e and magit are complaining about transient so just load it from straight at the beginning
and forget about it.
** Transient
#+begin_src emacs-lisp
...
With this set up M-x will hide commands that don't correspond with
current mode. e.g. it wont show you SLY commands in rust-mode, etc.

There is this `#'command-completion-using-modes-and-keymaps-p` and it's more
aggressive. It will show commands that are tagged for the buffer plus
any additional keybindings for that mode. `#'command-completion-default-include-p`
is the safe option as excludes commands that are tagged for other modes and 
includes everything else. See more on:
...
#+end_src
* Magit
This is the best thing man kind created right after C-u.
...
* Modes
** Repeat mode
This is a great thing that was introduced in Emacs 28+. What is does
it enables you that instead of going `C-x o C-x o C-x o...` you just
do `C-x o o o o` as many times as you want. This also work with other
repetative keybindings as well! To check all key mappings that support 
repeat-mode do `M-x describe-repeat-maps RET`. 
...
** EXWM
These are sound and brightness controls
